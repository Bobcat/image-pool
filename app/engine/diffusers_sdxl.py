...
class DiffusersSdxlRuntime:
    def __init__(self, model_name: str, settings: ModelSettings) -> None:
        ...
        try:
            ...
            img2img_pipe = AutoPipelineForImage2Image.from_pipe(pipe).to(self._device)
    ...
    def _complete_sync(self, job: ImageJob) -> ImageResult:
        if job.operation not in {"generation", "edit"}:
            raise ValueError("diffusers_sdxl only supports image generation and image edit")

        started_at = time.perf_counter()
        width, height = _parse_size(job.size)
        steps = _metadata_int(job.metadata, "steps", default=self.settings.recommended_steps or 30, minimum=1, maximum=80)
        guidance = _metadata_float(
            job.metadata,
            "guidance",
            default=self.settings.recommended_guidance or 5.0,
            minimum=0.0,
            maximum=20.0,
        )
        ...
        for index in range(job.n):
            ...
            pipe = self._img2img_pipe if input_image is not None else self._pipe
...
def _decode_image(job: ImageJob) -> Image.Image:
    if len(job.images) != 1:
        raise ValueError("diffusers_sdxl image edit requests require exactly one input image")
